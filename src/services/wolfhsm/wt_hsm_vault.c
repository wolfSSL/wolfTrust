...
static int wt_hsm_vault_same_identity(const uint8_t* a, const uint8_t* b)
{
    return memcmp(a, b, 16U) == 0 &&
           memcmp(a + 20U, b + 20U, 4U) == 0;
}
...
static psa_status_t wt_hsm_vault_recover(wt_hsm_vault_table_t* table)
{
    ...
    rc = wh_Nvm_GetMetadata(g_vault_nvm, WT_HSM_VAULT_STAGE_ID,
                            &stage_meta);
    if (rc != WH_ERROR_OK ||
            !wt_hsm_vault_same_identity(meta.label, stage_meta.label)) {
        return PSA_ERROR_STORAGE_FAILURE;
    }
                                     ...
                                     const uint8_t* data, size_t len)
{
    ...
    if (replacement != 0) {
        ...
        status = wt_hsm_vault_map_err(
            wh_Nvm_AddObject(g_vault_nvm, &stage_meta, meta.len,
                             g_vault_ct));
        if (status != PSA_SUCCESS) {
            return status;
        }
